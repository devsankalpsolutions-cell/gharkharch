...
import { House, PaymentMethod } from '@/lib/types';
import { useFinance } from '@/context/FinanceContext';
import { Sparkles, CheckSquare, Square } from 'lucide-react';

interface HouseSettlementModalProps {
  isOpen: boolean;
  onClose: () => void;
  house: House;
...
const PAYMENT_METHODS: PaymentMethod[] = ['UPI', 'Bank Transfer', 'Cash'];

export const HouseSettlementModal: React.FC<HouseSettlementModalProps> = ({
  isOpen,
  onClose,
  house,
  initialData,
}) => {
  const { addHouseSettlement } = useFinance();

  const [fromId, setFromId] = useState(house.members[0]?.id || '');
  const [toId, setToId] = useState(house.members[1]?.id || '');
